...
const ProductListPage = ({ getProducts, categories, subCat }) => {
  ...
  useEffect(() => {
    getProducts(categoryName, pageNumParam, searchQuery, filters, sortOption, subCat)
      .then((products) => {
  ...
  return (
    <Container>
      <Row>
        ...
        <Col md={9}>
          <Row className="m-2" xs={1} md={2} lg={3}>
            {loading ? (
